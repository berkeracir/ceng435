...
if [ -n "$USERNAME" ]; then
	if [ -n "$SSH_KEY" ]; then
		if [ -f "$SSH_KEY" ]; then
            ...
            gnome-terminal -x bash -c "ssh -X -t -i $SSH_KEY -p $B_PORT $USERNAME@$REMOTE \"sudo route add -net 10.10.3.0 netmask 255.255.255.0 dev eth2; sudo route add -net 10.10.5.0 netmask 255.255.255.0 dev eth3\""
            ...
            gnome-terminal -x bash -c "ssh -X -t -i $SSH_KEY -p $D_PORT $USERNAME@$REMOTE \"sudo route add -net 10.10.2.0 netmask 255.255.255.0 dev eth1; sudo route add -net 10.10.4.0 netmask 255.255.255.0 dev eth2\""
